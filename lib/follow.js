...
function checkFollower ({registry, client, changes}, cb) {
  request({
    url: registry.url,
    json: true
  }, (err, resp, data) => {
    if (err || resp.statusCode !== 200) return cb()
    client.get(registry.seqKey, (err, reply) => {
      const seq = parseInt(reply, 10)
      if (err || isNaN(seq) || data.update_seq >= seq) return cb()
      changes.destroy()
      client.set(registry.seqKey, Math.max(0, data.update_seq - 3000), err => {
        if (err) rollbar.handleErrorWithPayloadData(err, data)
        cb(true)
      })
    })
  })
